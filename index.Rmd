...
.center[<img src="figure/measure.png" width="60%" align="center" />]
...
```{r}
ddy <- read_idf(file.path("materials/USA_CA_San.Francisco.Intl.AP.724940_TMY3.ddy"), 8.8)
...
```{r run, out.lines = 14}
path_epw <- file.path("materials/USA_CA_San.Francisco.Intl.AP.724940_TMY3.epw")
idf <- read_idf("materials/5ZoneWaterLoopHeatPump.idf")
...
job$report_data(dict[is_meter == 1])
```
...
```{r}
(job$report_data(tz = "Asia/Singapore")$datetime)
```
...
Get the total cooling rate power of AHU named `Heat Pumpa Cooling Mode AHU1` at
