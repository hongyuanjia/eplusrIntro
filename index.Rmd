...
extra_css <- list(
  ...
  ".scroll-output" = list(height = "80%", "overflow-y" = "scroll")
)
...
- **`Group`** 
    - `r icon::fa("sort-amount-down")` **`Class`**
         - `r icon::fa("sort-amount-down")` **`Object`**
             - `r icon::fa("sort-amount-down")` **`Field`**
                  - `r icon::fa("sort-amount-down")` **`Value`** 
...
.blue[\[`r icon::fa("question", size = 2)`\] If so, how can I get distinguish objects that do not have names?]
...
## Using text editor, error checking. programically changing
...
## What is Group? Class? Object? Field? Value?

<img src="figure/nomenclature.png" width="95%" align="center" />
...
## I can directly read my results from excel, why bother using eplusr?
Parametric simulations
Read all data into memory is expensive.

`$report_data()` using SQL

---

## I can do all of this in IDF Editor, why bother `Idf` class?
...
## I can directly read from Exel. The date time column. the start day of week
...
## Typical Workflow for A Building Energy Modeller
...
## Read EnergyPlus Model

***How can I read an EnergyPlus model?***
